...
fun createChart(
    ...
    val input = chart.addSeries("Before", inputData.project(0), inputData.project(1))
    input.lineStyle = SeriesLines.NONE
    input.marker = SeriesMarkers.CIRCLE

    val test = chart.addSeries("After", outputData.project(0), outputData.project(1))
...
fun main() {
    ...
    val network = Track(trainingData, 4000, 100).singleNetwork(15)
    ...
    val chart = createChart("test123", testData.output(), testData.input(), outputData)
